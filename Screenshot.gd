...
func _capture_thread(_arg):
	...
	# Print path to image (just for debugging - remove if you like)
	var os_separator = "\\" if OS.get_name() == "Windows" else "/"
	var os_path = PoolStringArray([OS.get_user_data_dir(), _screenshot_directory, image_filename]).join(os_separator)
